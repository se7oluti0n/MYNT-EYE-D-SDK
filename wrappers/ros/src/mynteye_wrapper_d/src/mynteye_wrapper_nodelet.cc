...
#include <mynteyed_ros2_msgs/srv/get_params.hpp>
...
#include <mynteyed_ros2_msgs/msg/temp.hpp> // NOLINT
...
public:
  ...
  rclcpp::Publisher<mynteyed_ros2_msgs::msg::Temp>::SharedPtr pub_temp;
  rclcpp::Publisher<sensor_msgs::msg::Imu>::SharedPtr pub_imu_processed;
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr
      pub_mesh_; // < The publisher for camera mesh.
  ...
  MYNTEYEWrapperNodelet(const rclcpp::NodeOptions & options) : rclcpp::Node(options){
    skip_tag = -1;
    skip_tmp_left_tag = 0;
  ...
  void onInit() {
    ...
    this->declare_parameter("dev_index", dev_index);
    this->declare_parameter("framerate", framerate);
    this->declare_parameter("ros_output_framerate", ros_output_framerate);
    this->declare_parameter("dev_mode", dev_mode);
    this->declare_parameter("color_mode", color_mode);
    this->declare_parameter("depth_mode", depth_mode);
    this->declare_parameter("stream_mode", stream_mode);
    this->declare_parameter("color_stream_format", color_stream_format);
    this->declare_parameter("depth_stream_format", depth_stream_format);
    this->declare_parameter("state_ae", state_ae);
    this->declare_parameter("state_awb", state_awb);
    this->declare_parameter("ir_intensity", ir_intensity);
    this->declare_parameter("ir_depth_only", ir_depth_only);
    this->declare_parameter("depth_type", depth_type);
    this->declare_parameter("imu_timestamp_align", imu_timestamp_align);

    this->get_parameter("dev_index", dev_index);
    this->get_parameter("framerate", framerate);
    this->get_parameter("ros_output_framerate", ros_output_framerate);
    this->get_parameter("dev_mode", dev_mode);
    this->get_parameter("color_mode", color_mode);
    this->get_parameter("depth_mode", depth_mode);
    this->get_parameter("stream_mode", stream_mode);
    this->get_parameter("color_stream_format", color_stream_format);
    this->get_parameter("depth_stream_format", depth_stream_format);
    this->get_parameter("state_ae", state_ae);
    this->get_parameter("state_awb", state_awb);
    this->get_parameter("ir_intensity", ir_intensity);
    this->get_parameter("ir_depth_only", ir_depth_only);
    this->get_parameter("depth_type", depth_type);
    this->get_parameter("imu_timestamp_align", imu_timestamp_align);
    ...
    this->declare_parameter("points_frequency", points_frequency);
    this->declare_parameter("points_factor", points_factor);
    this->declare_parameter("gravity", gravity);
    this->get_parameter("points_frequency", points_frequency);
    this->get_parameter("points_factor", points_factor);
    ...
    this->declare_parameter("base_frame_id", base_frame_id);
    this->declare_parameter("left_mono_frame", left_mono_frame_id);
    this->declare_parameter("left_color_frame", left_color_frame_id);
    this->declare_parameter("right_mono_frame", right_mono_frame_id);
    this->declare_parameter("right_color_frame", right_color_frame_id);
    this->declare_parameter("depth_frame", depth_frame_id);
    this->declare_parameter("points_frame", points_frame_id);
    this->declare_parameter("imu_frame", imu_frame_id);
    this->declare_parameter("temp_frame", temp_frame_id);
    this->declare_parameter("imu_frame_processed", imu_frame_processed_id);


    this->get_parameter("base_frame_id", base_frame_id);
    this->get_parameter("left_mono_frame", left_mono_frame_id);
    this->get_parameter("left_color_frame", left_color_frame_id);
    this->get_parameter("right_mono_frame", right_mono_frame_id);
    ...
    this->get_parameter("temp_frame", temp_frame_id);
    this->get_parameter("imu_frame_processed", imu_frame_processed_id);

    RCLCPP_INFO_STREAM(get_logger(), "base_frame: " << base_frame_id);
    RCLCPP_INFO_STREAM(get_logger(), "left_mono_frame: " << left_mono_frame_id);
    RCLCPP_INFO_STREAM(get_logger(), "left_color_frame: " << left_color_frame_id);
    RCLCPP_INFO_STREAM(get_logger(), "right_mono_frame: " << right_mono_frame_id);
    RCLCPP_INFO_STREAM(get_logger(), "right_color_frame: " << right_color_frame_id);
    RCLCPP_INFO_STREAM(get_logger(), "depth_frame: " << depth_frame_id);
    RCLCPP_INFO_STREAM(get_logger(), "points_frame: " << points_frame_id);
    RCLCPP_INFO_STREAM(get_logger(), "imu_frame: " << imu_frame_id);
    RCLCPP_INFO_STREAM(get_logger(), "temp_frame: " << temp_frame_id);
    RCLCPP_INFO_STREAM(get_logger(), "imu_frame_processed: " << imu_frame_processed_id);
    ...
    this->declare_parameter("left_mono_topic", left_mono_topic);
    this->declare_parameter("left_color_topic", left_color_topic);
    this->declare_parameter("right_mono_topic", right_mono_topic);
    this->declare_parameter("right_color_topic", right_color_topic);
    this->declare_parameter("depth_topic", depth_topic);
    this->declare_parameter("points_topic", points_topic);
    this->declare_parameter("imu_topic", imu_topic);
    this->declare_parameter("temp_topic", temp_topic);
    this->declare_parameter("imu_processed_topic", imu_processed_topic);

    this->get_parameter("left_mono_topic", left_mono_topic);
    this->get_parameter("left_color_topic", left_color_topic);
    this->get_parameter("right_mono_topic", right_mono_topic);
    ...
    this->get_parameter("temp_topic", temp_topic);
    ...
    mynteye.reset(new Camera);
    {
      ...
      RCLCPP_INFO_STREAM(get_logger(), dashes);
      RCLCPP_INFO_STREAM(get_logger(), "Device Information");
      RCLCPP_INFO_STREAM(get_logger(), dashes);
      for (auto &&info : dev_infos) {
        RCLCPP_INFO_STREAM(get_logger(), info.index << " | " << info);
      }
      RCLCPP_INFO_STREAM(get_logger(), dashes);
    ...
    spec_version = mynteye->GetDescriptors()->spec_version;
    {
      ...
      RCLCPP_INFO_STREAM(get_logger(), "Color Stream Information");
      RCLCPP_INFO_STREAM(get_logger(), dashes);
      ...
      RCLCPP_INFO_STREAM(get_logger(), "Depth Stream Information");
      RCLCPP_INFO_STREAM(get_logger(), dashes);
    ...
    pub_mesh_ = this->create_publisher<visualization_msgs::msg::Marker>("camera_mesh", 1);
    ...
    this->declare_parameter("mesh_file", mesh_file);
    if (this->get_parameter("mesh_file", mesh_file)) {
      mesh_msg_.mesh_resource = "package://mynteye_wrapper_d/mesh/" + mesh_file;
    } else {
      RCLCPP_INFO_STREAM(get_logger(), "no mesh found for visualisation, set ros param "
                          "mesh_file, if desired");
    ...
    RCLCPP_INFO_STREAM(get_logger(), "Advertized service " << DEVICE_PARAMS_SERVICE);
    ...
    params.color_stream_format = static_cast<StreamFormat>(color_stream_format);
    params.depth_stream_format = static_cast<StreamFormat>(depth_stream_format);
    ...
    image_transport::ImageTransport it_mynteye(this);
    // left
    pub_left_mono = it_mynteye.advertise(left_mono_topic, 1);
    RCLCPP_INFO_STREAM(get_logger(), "Advertized on topic " << left_mono_topic);
    pub_left_color = it_mynteye.advertiseCamera(left_color_topic, 1);
    RCLCPP_INFO_STREAM(get_logger(), "Advertized on topic " << left_color_topic);
    // right
    pub_right_mono = it_mynteye.advertise(right_mono_topic, 1);
    RCLCPP_INFO_STREAM(get_logger(), "Advertized on topic " << right_mono_topic);
    pub_right_color = it_mynteye.advertiseCamera(right_color_topic, 1);
    RCLCPP_INFO_STREAM(get_logger(), "Advertized on topic " << right_color_topic);
    // depth
    pub_depth = it_mynteye.advertiseCamera(depth_topic, 1);
    RCLCPP_INFO_STREAM(get_logger(), "Advertized on topic " << depth_topic);
    // points
    pub_points = this->create_publisher<sensor_msgs::PointCloud2>(points_topic, 1);
    RCLCPP_INFO_STREAM(get_logger(), "Advertized on topic " << points_topic);
    // imu
    pub_imu = this->create_publisher<sensor_msgs::msg::Imu>(imu_topic, 100);
    RCLCPP_INFO_STREAM(get_logger(), "Advertized on topic " << imu_topic);
    pub_imu_processed =
        this->create_publisher<sensor_msgs::msg::Imu>(imu_processed_topic, 100); // NOLINT
    RCLCPP_INFO_STREAM(get_logger(), "Advertized on topic " << imu_processed_topic);
    // temp
    pub_temp = this->create_publisher<mynteyed_ros2_msgs::msg::Temp>(temp_topic, 100);
    RCLCPP_INFO_STREAM(get_logger(), "Advertized on topic " << temp_topic);
    ...
    rclcpp::Rate loop_rate(framerate);
    while (rclcpp::ok()) {
      publishMesh();
      detectSubscribers();
      loop_rate.sleep();
  ...
  void openDevice() {
    ...
    mynteye->SetMotionCallback([this](const MotionData &data) {
      ...
      pthread_mutex_unlock(&mutex_sub_result);
      if (data.imu &&
          (sub_result_imu || sub_result_imu_processed || sub_result_temp)) {
        if (data.imu->flag == MYNTEYE_IMU_ACCEL) {
          imu_accel = data.imu;
          if (imu_timestamp_align) {
            publishAlignImu(sub_result_imu, sub_result_imu_processed,
                            sub_result_temp);
          } else {
            publishImu(sub_result_imu, sub_result_imu_processed,
                       sub_result_temp);
          }
    ...
    RCLCPP_INFO_STREAM(get_logger(), "Open camera success");

    // camera infos
    bool in_ok;
    auto &&in = mynteye->GetStreamIntrinsics(params.stream_mode, &in_ok);
    if (in_ok) {
      RCLCPP_INFO_STREAM(get_logger(), "Camera info is created");
    } else {
  ...
  void publishLeft(const StreamData &data, const rclcpp::Time &timestamp,
    ...
    publishColor(data, timestamp, left_info_ptr, pub_left_color, color_sub,
                 left_color_frame_id, pub_left_mono, mono_sub,
                 left_mono_frame_id, true);
  }

  void publishRight(const StreamData &data, const rclcpp::Time &timestamp,
                    bool color_sub, bool mono_sub) {
    if (skip_tag > 0) {
    ...
    publishColor(data, timestamp, right_info_ptr, pub_right_color, color_sub,
                 right_color_frame_id, pub_right_mono, mono_sub,
                 right_mono_frame_id, false);
  }
  ...
  void publishDepth(const StreamData &data, const rclcpp::Time &timestamp) {
    std_msgs::Header header;
    header.stamp = timestamp;
  ...
  void publishPoints(rclcpp::Time stamp) {
  ...
  void timestampAlign() {
    ...
    if (imu_accel != nullptr) {
      ...
      if (acc.timestamp <= gyro.timestamp) {
        ...
        acc_temp.accel[0] =
            acc.accel[0] + (imu_accel->accel[0] - acc.accel[0]) * k;
        acc_temp.accel[1] =
            acc.accel[1] + (imu_accel->accel[1] - acc.accel[1]) * k;
        acc_temp.accel[2] =
            acc.accel[2] + (imu_accel->accel[2] - acc.accel[2]) * k;
  ...
  void publishAlignImu(bool imu_sub, bool imu_processed_sub, bool temp_sub) {
    ...
    rclcpp::Time stamp = checkUpImuTimeStamp(imu_accel->timestamp);

    if (imu_sub) {
      auto msg = getImuMsgFromData(this->now(), imu_frame_id,
                                   *imu_accel, *imu_gyro);
    ...
    if (motion_intrinsics_enabled && imu_processed_sub) {
      ...
      auto msg = getImuMsgFromData(this->now(), imu_frame_processed_id,
                                   data_acc2, data_gyr2);
      msg.header.stamp = stamp;
    ...
    if (temp_sub) {
      auto msg =
          getTempMsgFromData(this->now(), temp_frame_id, *imu_accel);
      msg.header.stamp = stamp;
  ...
  void publishImu(bool imu_sub, bool imu_processed_sub, bool temp_sub) {
    if (imu_accel == nullptr || imu_gyro == nullptr) {
      return;
    }
    ...
    if (motion_intrinsics_enabled && imu_processed_sub) {
      ...
      auto data_acc2 = ProcImuAssembly(data_acc1);
      auto data_gyr2 = ProcImuAssembly(data_gyr1);
      auto msg = getImuMsgFromData(rclcpp::Time::now(), imu_frame_processed_id,
                                   data_acc2, data_gyr2);
      msg.header.stamp = stamp;
      pub_imu_processed->publish(msg);
    }
    ...
    if (temp_sub) {
      auto msg =
          getTempMsgFromData(rclcpp::Time::now(), temp_frame_id, *imu_accel);
      msg.header.stamp = stamp;
      pub_temp->publish(msg);
    }
  ...
  sensor_msgs::msg::Imu getImuMsgFromData(rclcpp::Time stamp,
                                          const std::string &frame_id,
                                          const ImuData &imu_accel,
                                          const ImuData &imu_gyro) {
  ...
  mynteye_wrapper_d::Temp getTempMsgFromData(rclcpp::Time stamp,
                                             const std::string &frame_id,
                                             const ImuData &imu_accel) {
  ...
  rclcpp::Time hardTimeToSoftTime(std::uint64_t _hard_time) {
    ...
    if (false == isInited) {
      soft_time_begin = this->now().seconds();
    ...
    double time_sec_double =
        rclcpp::Time(time_ns_detal_s, time_ns_detal_ns * 10000).toSec();
  ...
  rclcpp::Time checkUpTimeStamp(std::uint64_t _hard_time,
    ...
    return hardTimeToSoftTime(image_acc[type] * unit_hard_time + _hard_time);
  }
  ...
  rclcpp::Time checkUpImuTimeStamp(std::uint64_t _hard_time) {
    ...
    return hardTimeToSoftTime(imu_acc * unit_hard_time + _hard_time);
  }
  ...
  ImuData ProcImuAssembly(const ImuData &data) const {
    ...
    if (nullptr == motion_intrinsics) {
      std::cout << "[WARNING!] Motion intrinsic "
                << "is not been supported at this device." << std::endl;
      return res;
    }
    double dst[3][3] = {0};
    if (data.flag == 1) {
  ...
  ImuData ProcImuTempDrift(const ImuData &data) const {
    ...
    if (res.flag == 1) {
      res.accel[0] -=
          motion_intrinsics->accel.x[1] * temp + motion_intrinsics->accel.x[0];
      res.accel[1] -=
          motion_intrinsics->accel.y[1] * temp + motion_intrinsics->accel.y[0];
      res.accel[2] -=
          motion_intrinsics->accel.z[1] * temp + motion_intrinsics->accel.z[0];
    } else if (res.flag == 2) {
      res.gyro[0] -=
          motion_intrinsics->gyro.x[1] * temp + motion_intrinsics->gyro.x[0];
      res.gyro[1] -=
          motion_intrinsics->gyro.y[1] * temp + motion_intrinsics->gyro.y[0];
      res.gyro[2] -=
          motion_intrinsics->gyro.z[1] * temp + motion_intrinsics->gyro.z[0];
    } else if (res.flag == 11) {
      res.accel[0] -=
          motion_intrinsics->accel.x[1] * temp + motion_intrinsics->accel.x[0];
      res.accel[1] -=
  ...
  void publishMesh() {
    ...
    mesh_msg_.header.stamp = this->now();
    mesh_msg_.type = visualization_msgs::msg::Marker::MESH_RESOURCE;

    double Pie = 3.1416;
    tf2::Quaternion q;
    q.setRPY(Pie / 2, 0.0, Pie);  // angles in radians
    // fill orientation
    ...
    mesh_msg_.action = visualization_msgs::msg::Marker::ADD;
  ...
  //bool getParams(mynteye_wrapper_d::GetParams::Request &req,    // NOLINT
  ...
  //    bool in_ok;
  //    auto &&in_vga = mynteye->GetStreamIntrinsics(StreamMode::STREAM_1280x480,
  //                                                 &in_ok_1); // NOLINT
  //    auto &&in_hd = mynteye->GetStreamIntrinsics(StreamMode::STREAM_2560x720,
  //                                                &in_ok_2); // NOLINT
  ...
  //               {"cy", in_vga.left.cy},
  //               {"coeffs",
  //                Config::array({in_vga.left.coeffs[0], in_vga.left.coeffs[1],
  //                               in_vga.left.coeffs[2], in_vga.left.coeffs[3],
  //                               in_vga.left.coeffs[4]})},
  //               {"p", Config::array({in_vga.left.p[0], in_vga.left.p[1],
  //                                    in_vga.left.p[2], // NOLINT
  //                                    in_vga.left.p[3], in_vga.left.p[4],
  //                                    in_vga.left.p[5], // NOLINT
  //                                    in_vga.left.p[6], in_vga.left.p[7],
  //                                    in_vga.left.p[8], // NOLINT
  //                                    in_vga.left.p[9], in_vga.left.p[10],
  //                                    in_vga.left.p[11]})} // NOLINT
  //           }},
  //          {"right",
  //           {
  ...
  //               {"cy", in_vga.right.cy},
  //               {"coeffs",
  //                Config::array({in_vga.right.coeffs[0], in_vga.right.coeffs[1],
  //                               in_vga.right.coeffs[2], in_vga.right.coeffs[3],
  //                               in_vga.right.coeffs[4]})},
  //               {"p", Config::array({in_vga.right.p[0], in_vga.right.p[1],
  //                                    in_vga.right.p[2], // NOLINT
  //                                    in_vga.right.p[3], in_vga.right.p[4],
  //                                    in_vga.right.p[5], // NOLINT
  //                                    in_vga.right.p[6], in_vga.right.p[7],
  //                                    in_vga.right.p[8], // NOLINT
  //                                    in_vga.right.p[9], in_vga.right.p[10],
  //                                    in_vga.right.p[11]})} // NOLINT
  //           }}};
  //      std::string json = dump_string(intrinsics, JSON);
  //      res.value = json;
  //    } else if (params.stream_mode == StreamMode::STREAM_2560x720 && in_ok_2) {
  //      Config intrinsics{
  //          {"calib_model", "pinhole"},
  //          {"left",
  //           {
  ...
  //               {"cy", in_hd.left.cy},
  //               {"coeffs",
  //                Config::array({in_hd.left.coeffs[0], in_hd.left.coeffs[1],
  //                               in_hd.left.coeffs[2], in_hd.left.coeffs[3],
  //                               in_hd.left.coeffs[4]})},
  //               {"p", Config::array({in_hd.left.p[0], in_hd.left.p[1],
  //                                    in_hd.left.p[2], // NOLINT
  //                                    in_hd.left.p[3], in_hd.left.p[4],
  //                                    in_hd.left.p[5], // NOLINT
  //                                    in_hd.left.p[6], in_hd.left.p[7],
  //                                    in_hd.left.p[8], // NOLINT
  //                                    in_hd.left.p[9], in_hd.left.p[10],
  //                                    in_hd.left.p[11]})} // NOLINT
  //           }},
  //          {"right",
  //           {
  ...
  //               {"cy", in_hd.right.cy},
  //               {"coeffs",
  //                Config::array({in_hd.right.coeffs[0], in_hd.right.coeffs[1],
  //                               in_hd.right.coeffs[2], in_hd.right.coeffs[3],
  //                               in_hd.right.coeffs[4]})},
  //               {"p", Config::array({in_hd.right.p[0], in_hd.right.p[1],
  //                                    in_hd.right.p[2], // NOLINT
  //                                    in_hd.right.p[3], in_hd.right.p[4],
  //                                    in_hd.right.p[5], // NOLINT
  //                                    in_hd.right.p[6], in_hd.right.p[7],
  //                                    in_hd.right.p[8], // NOLINT
  //                                    in_hd.right.p[9], in_hd.right.p[10],
  //                                    in_hd.right.p[11]})} // NOLINT
  //           }}};
  //      std::string json = dump_string(intrinsics, JSON);
  //      res.value = json;
  //    } else {
  ...
  //    auto vga_extrinsics = mynteye->GetStreamExtrinsics(
  //        StreamMode::STREAM_1280x480, &ex_ok_1); // NOLINT
  //    auto hd_extrinsics = mynteye->GetStreamExtrinsics(
  //        StreamMode::STREAM_2560x720, &ex_ok_2); // NOLINT
  //    if (params.stream_mode == StreamMode::STREAM_1280x480 && ex_ok_1) {
  //      Config extrinsics{
  //          {"rotation",
  //           Config::array(
  //               {vga_extrinsics.rotation[0][0], vga_extrinsics.rotation[0][1],
  //                vga_extrinsics.rotation[0][2], // NOLINT
  //                vga_extrinsics.rotation[1][0], vga_extrinsics.rotation[1][1],
  //                vga_extrinsics.rotation[1][2], // NOLINT
  //                vga_extrinsics.rotation[2][0], vga_extrinsics.rotation[2][1],
  //                vga_extrinsics.rotation[2][2]})}, // NOLINT
  //          {"translation",
  //           Config::array({vga_extrinsics.translation[0],
  //                          vga_extrinsics.translation[1],
  //                          vga_extrinsics.translation[2]})} // NOLINT
  //      };
  //      std::string json = dump_string(extrinsics, configuru::JSON);
  //      res.value = json;
  //    } else if (params.stream_mode == StreamMode::STREAM_2560x720 && ex_ok_2) {
  //      Config extrinsics{
  //          {"rotation",
  //           Config::array(
  //               {hd_extrinsics.rotation[0][0], hd_extrinsics.rotation[0][1],
  //                hd_extrinsics.rotation[0][2], // NOLINT
  //                hd_extrinsics.rotation[1][0], hd_extrinsics.rotation[1][1],
  //                hd_extrinsics.rotation[1][2], // NOLINT
  //                hd_extrinsics.rotation[2][0], hd_extrinsics.rotation[2][1],
  //                hd_extrinsics.rotation[2][2]})}, // NOLINT
  //          {"translation",
  //           Config::array({hd_extrinsics.translation[0],
  //                          hd_extrinsics.translation[1],
  //                          hd_extrinsics.translation[2]})} // NOLINT
  //      };
  //      std::string json = dump_string(extrinsics, configuru::JSON);
  //      res.value = json;
  //    } else {
  ...
  //               {"scale",
  //                Config::array(
  //                    {intri.accel.scale[0][0], intri.accel.scale[0][1],
  //                     intri.accel.scale[0][2], // NOLINT
  //                     intri.accel.scale[1][0], intri.accel.scale[1][1],
  //                     intri.accel.scale[1][2], // NOLINT
  //                     intri.accel.scale[2][0], intri.accel.scale[2][1],
  //                     intri.accel.scale[2][2]})}, // NOLINT
  //               {"assembly",
  //                Config::array(
  //                    {intri.accel.assembly[0][0], intri.accel.assembly[0][1],
  //                     intri.accel.assembly[0][2], // NOLINT
  //                     intri.accel.assembly[1][0], intri.accel.assembly[1][1],
  //                     intri.accel.assembly[1][2], // NOLINT
  //                     intri.accel.assembly[2][0], intri.accel.assembly[2][1],
  //                     intri.accel.assembly[2][2]})}, // NOLINT
  //               {"drift",
  //                Config::array({intri.accel.drift[0], intri.accel.drift[1],
  //                               intri.accel.drift[2]})}, // NOLINT
  //               {"noise",
  //                Config::array({intri.accel.noise[0], intri.accel.noise[1],
  //                               intri.accel.noise[2]})}, // NOLINT
  //               {"bias",
  //                Config::array({intri.accel.bias[0], intri.accel.bias[1],
  //                               intri.accel.bias[2]})}, // NOLINT
  //               {"x", Config::array(
  //                         {intri.accel.x[0], intri.accel.x[1]})}, // NOLINT
  //               {"y", Config::array(
  //                         {intri.accel.y[0], intri.accel.y[1]})}, // NOLINT
  //               {"z",
  //                Config::array({intri.accel.z[0], intri.accel.z[1]})} // NOLINT
  //           }},
  //          {"gyro",
  //           {
  //               {"scale",
  //                Config::array({intri.gyro.scale[0][0], intri.gyro.scale[0][1],
  //                               intri.gyro.scale[0][2], // NOLINT
  //                               intri.gyro.scale[1][0], intri.gyro.scale[1][1],
  //                               intri.gyro.scale[1][2], // NOLINT
  //                               intri.gyro.scale[2][0], intri.gyro.scale[2][1],
  //                               intri.gyro.scale[2][2]})}, // NOLINT
  //               {"assembly",
  //                Config::array(
  //                    {intri.gyro.assembly[0][0], intri.gyro.assembly[0][1],
  //                     intri.gyro.assembly[0][2], // NOLINT
  //                     intri.gyro.assembly[1][0], intri.gyro.assembly[1][1],
  //                     intri.gyro.assembly[1][2], // NOLINT
  //                     intri.gyro.assembly[2][0], intri.gyro.assembly[2][1],
  //                     intri.gyro.assembly[2][2]})}, // NOLINT
  //               {"drift",
  //                Config::array({intri.gyro.drift[0], intri.gyro.drift[1],
  //                               intri.gyro.drift[2]})}, // NOLINT
  //               {"noise",
  //                Config::array({intri.gyro.noise[0], intri.gyro.noise[1],
  //                               intri.gyro.noise[2]})}, // NOLINT
  //               {"bias", Config::array({intri.gyro.bias[0], intri.gyro.bias[1],
  //                                       intri.gyro.bias[2]})}, // NOLINT
  //               {"x",
  //                Config::array({intri.gyro.x[0], intri.gyro.x[1]})}, // NOLINT
  //               {"y",
  //                Config::array({intri.gyro.y[0], intri.gyro.y[1]})}, // NOLINT
  //               {"z",
  //                Config::array({intri.gyro.z[0], intri.gyro.z[1]})} // NOLINT
  //           }}};
  //      std::string json = dump_string(intrinsics, JSON);
  //      res.value = json;
  //    } else {
  //      RCLCPP_INFO_STREAM(get_logger(), "INVALID IMU INTRINSICS");
  //      res.value = "null";
  //    }
  //  } break;
  ...
  //      Config extrinsics{
  //          {"rotation",
  //           Config::array({extri.rotation[0][0], extri.rotation[0][1],
  //                          extri.rotation[0][2], // NOLINT
  //                          extri.rotation[1][0], extri.rotation[1][1],
  //                          extri.rotation[1][2], // NOLINT
  //                          extri.rotation[2][0], extri.rotation[2][1],
  //                          extri.rotation[2][2]})}, // NOLINT
  //          {"translation",
  //           Config::array({extri.translation[0], extri.translation[1],
  //                          extri.translation[2]})} // NOLINT
  //      };
  //      std::string json = dump_string(extrinsics, configuru::JSON);
  //      res.value = json;
  //    } else {
  //      RCLCPP_INFO_STREAM(get_logger(), "INVALID IMU EXTRINSICS");
  //      res.value = "null";
  //    }
  //  } break;
  //  default:
...
RCLCPP_COMPONENTS_REGISTER_NODE(MYNTEYE_NAMESPACE::MYNTEYEWrapperNodelet)
